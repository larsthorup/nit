...
class Index {
  constructor({ indexPath }) {
    assert(indexPath instanceof Path);
  ...
  add({ name, oid, stat }) {
    ...
    assert(oid instanceof Oid);
    assert(stat instanceof fs.Stats);
    const entry = new Entry({ name, oid, stat });
    this.entryByName[name.value] = entry;
  }

  async writingUpdates() {
    const version = 2;
    const nameList = Object.keys(this.entryByName);
    await Lockfile.tryHolding(
      { filePath: this.indexPath.value },
      async lockfile => {
