...
class Index {
  ...
  discardConflicts({ entry }) {
    // Note: to allow a file in the index, none of its parent directories can be in the index
    const { parentDirectoryList } = entry.name.split();
    while (parentDirectoryList.length > 0) {
      const name = Name.fromDirectoryList(parentDirectoryList);
      this.discardEntry({ name });
