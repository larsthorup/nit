...
class Index {
  ...
  getEntry({ name }) {
    return this.entryByName[name.value];
  }
  ...
  isTracked({ name }) {
    const isTrackedFile = this.isTrackedFile({ name });
    const isDirectoryWithTrackedFile = this.entryNameSetByParentName.hasOwnProperty(
      name.value
    );
    return isTrackedFile || isDirectoryWithTrackedFile;
  }
