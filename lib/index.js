...
const { ChecksumReader } = require('./index/checksumReader');
...
class IndexEntry {
  ...
  static decode({ data }) {
    ...
    entry.name = new Name(data.slice(62).toString('utf8'));
    return entry;
  }
...
class Index {
  ...
  async loading({ lockfile }) {
    this.clear();
    const { fileDescriptor } = await this.opening();
    if (fileDescriptor) {
      const reader = new ChecksumReader({ fileDescriptor });
      ...
      await this.closing({ fileDescriptor });
    }
  }
  ...
  async opening() {
    try {
      const fileDescriptor = await util.promisify(fs.open)(this.path.value);
  ...
  async writing({ lockfile }) {
    ...
    console.log('writing checksum', calculatedChecksum);
    await lockfile.writingBinary(calculatedChecksum);
  }
}
