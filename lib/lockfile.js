...
class Lockfile {
  ...
  lock() {
    try {
      if (!this.lockfileDescriptor) {
        ...
        this.lockfileDescriptor = fs.openSync(this.lockPath.value, flags);
      }
      return true;
    } catch (error) {
      switch (error.code) {
  ...
  commit() {
    ...
    fs.renameSync(this.lockPath.value, this.path.value); // Note: atomicly replace the file
    delete this.lockfileDescriptor;
  }

  rollback() {
    this.ensureLocked();
    fs.closeSync(this.lockfileDescriptor); // ToDo: class File (wraps FileDescriptor and fs)
    fs.unlinkSync(this.lockPath.value);
    delete this.lockfileDescriptor;
  }
