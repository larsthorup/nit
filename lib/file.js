...
class File {
  ...
  static async writingFile({ path, content }) {
    return util.promisify(fs.writeFile)(path.value, content);
  }

  static async readingFile({ encoding, path }) {
    return {
      content: await util.promisify(fs.readFile)(path.value, { encoding }),
    };
  }
